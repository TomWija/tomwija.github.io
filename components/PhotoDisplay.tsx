...
export const PhotoDisplay: React.FC<PhotoDisplayProps> = ({ event }) => {
  ...
  return (
    <div className="flex-1 h-full relative overflow-hidden bg-stone-100 flex items-center justify-center p-4 md:p-12">
      ...
      <div className={`relative z-10 w-full max-w-4xl transition-all duration-700 ease-out transform ${animate ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        ...
        <div className="bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row max-h-[80vh] md:max-h-[600px]">
          ...
          <div className="w-full md:w-3/5 h-64 md:h-auto relative overflow-hidden group">
            ...
            <div className="absolute bottom-4 left-4 text-white md:hidden">
                <p className="text-xs font-bold uppercase tracking-widest opacity-80 mb-1">{event.date}</p>
          ...
          <div className="w-full md:w-2/5 p-6 md:p-10 flex flex-col justify-center bg-white relative">
            <div className={`transition-all duration-700 delay-300 ${animate ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-4'}`}>
              ...
              <p className={`text-stone-600 leading-relaxed ${event.isSpecial ? 'text-lg md:text-xl font-medium text-rose-600 italic' : 'text-base md:text-lg'}`}>
                {event.description}
              </p>
